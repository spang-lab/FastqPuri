...
if(all(data$lowQ_ACGT_tile == 0)){
   ...
   for (lpos in lanes) {
      ...
      pheatmap((data$lowQ_ACGT_tile/data$ACGT_tile)[tiles,1:4],cluster_cols=F, 
         cluster_rows=F, fontsize_row=6, fontsize_col=6,  color = color,
...
for (lpos in lanes) {
   ...
   if (any(is.nan(tiles_meanQ))){
      ...
      pheatmap(tiles_meanQ[tiles,],cluster_cols=F, cluster_rows=F, 
...
# modifications by Julia to have color scale range from 0 to 1:
